...
fn test_sst_build_all() {
    ...
    println!("Number of blocks {}", sst.block_meta.len());
    let block_meta = sst.block_meta.first().unwrap();
    println!(
        "Block first key {:?} last key {:?}",
        block_meta.first_key, block_meta.last_key
    );
    assert_eq!(
...
fn test_sst_seek_key() {
    ...
    for (i, block_meta) in sst.block_meta.iter().enumerate() {
        println!(
            "Block {} first {:?} last {:?}",
            i, block_meta.first_key, block_meta.last_key
        );
    }
    ...
    for offset in 1..=5 {
        for i in 0..num_of_keys() {
            ...
            .unwrap();
            println!("Seek to key {:03}", i * 5 + offset);
        }
